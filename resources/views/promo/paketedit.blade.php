...
<main class="flex-1 p-10">
    ...
    <div class="bg-white p-8 rounded shadow w-full max-w-full">
        <h2 class="text-lg font-semibold mb-6">
            <i class="fas fa-gift mr-2"></i> Edit Promo
        </h2>

        <form action="{{ route('paket.update', $paket->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 w-full">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label for="nama_paket" class="block text-sm font-medium text-gray-700">Promo Paket</label>
                ...
                {{-- Foto --}}
                <div>
                    <label for="foto" class="block text-sm font-medium text-gray-700">Foto Promo</label>
                    ...
                    @if ($paket->foto)
                        ...
                        <img src="{{ asset('storage/' . $paket->foto) }}" class="w-20 h-20 object-cover mt-1" alt="foto lama">
                    @endif
                    @error('foto')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
